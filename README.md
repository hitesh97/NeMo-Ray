...
- **Python pipeline** (`src/`, `config.yaml`, `out/`) — the Sionna RT coverage solve,
  cuOpt mast optimisation, RT verification, and the standalone deck.gl viewer (`viewer/`).
...
├── viewer/          # standalone deck.gl viewer (index.html, app.js, config.js)
...
3. A **deck.gl** viewer that renders the **extruded OSM buildings** (the same meshes the ray
   tracer uses) and overlays **animated ray traces** coloured by signal strength, EE masts,
   coverage holes, and cuOpt-proposed masts — all individually toggleable.
...
The pipeline publishes straight into the Next.js dashboard's static dir (served by Next at
`/raytracing/*`), so a fresh `python -m src.pipeline` run lands in the app with no copy step.
**Note:** the dashboard's 3D map surface is mid-migration — the CesiumJS map (and its
`RayTracingLayer`) was removed and the map is being rebuilt on **deck.gl** behind the map
seam, so these artifacts are not yet rendered in the HUD. See `nemoray/CLAUDE.md` ("Map")
for current status. The directory is gitignored — re-run the pipeline to regenerate it.
